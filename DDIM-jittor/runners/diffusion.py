...
class Diffusion(object):
    ...
    def sample(self):
        ...
        if not self.args.use_pretrained:
            if getattr(self.config.sampling, "ckpt_id", None) is None:
                states = jt.load(
                    os.path.join(self.args.log_path, "ckpt_33.pkl")
                )
            else:
                states = jt.load(
                    os.path.join(
    ...
    def test(self):
        ...
        # === 2. 计算 FID ===
        ref_dataset_name = args.fid_ref_dataset  # 例如 "cifar10_train"
        gen_folder = args.image_folder  # e.g., "./samples/cifar"

        # 示例：调用 clean-fid 命令行工具
        fid_cmd = f"clean-fid --mode legacy --batch-size 50 fid --dataset {ref_dataset_name} --gen {gen_folder}"
        print(f"\n[Running FID Evaluation]\n{fid_cmd}\n")
